...
//Rooms grid
var u_w = buffer_read(buffer, buffer_u8);
var u_h = buffer_read(buffer, buffer_u8);
...
for(var u_y = 0; u_y < u_h; u_y++)
    ...
    for(var u_x = 0; u_x < u_w; u_x++)
        {
        global.grid_room_types[# u_x, u_y] = buffer_read(buffer, buffer_u8);
        }
    }
...
var u_char_count = buffer_read(buffer, buffer_u8);
for(var u = 0; u < u_char_count; u++)
    {
    var u_grid_x = buffer_read(buffer, buffer_u8);
    var u_grid_y = buffer_read(buffer, buffer_u8);
